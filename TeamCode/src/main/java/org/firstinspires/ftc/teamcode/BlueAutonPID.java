...
//        strafe(-4, 0.5);
    ...
    private void strafe(double inches, double power) {
        ...
        int targetPos = Math.abs(rightFront.getCurrentPosition()) + (motorCounts);
...
//        pidStrafe.enable();
        ...
        while (opModeIsActive() && Math.abs(rightFront.getCurrentPosition()) < targetPos) {
            double correction = pidDrive.performPID(getAngle());
            double motorPower = pStartValue * (targetPos - Math.abs(rightFront.getCurrentPosition()));
...
//        sleep(2000);
        ...
        int motorCounts = (int) (forward * inches * COUNTS_PER_INCH);
        int targetPos = Math.abs(rightFront.getCurrentPosition()) + (motorCounts / 3);
        ...
        rightFront.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        rightFront.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        targetPos = Math.abs(rightFront.getCurrentPosition()) + (motorCounts * 2 / 3);
        pStartValue = 0.0005;
